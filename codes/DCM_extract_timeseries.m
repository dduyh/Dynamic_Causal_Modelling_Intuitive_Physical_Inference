...
timeseries_folder = '/Volumes/My_Passport/ETH_Zurich/Semester_Project/DCM/';
...
for i=1:nrSubjects
    ...
    save([data_folder 'S' subjectID{i} '/jobs_DCM_scrubbing/' 'extract_VOI_vis_batch.mat'],'matlabbatch');
    spm_jobman('run', matlabbatch)
    clear matlabbatch

    % extract timeseries for SPL VOI
    matlabbatch = extract_VOI_SPL(subjectID{i}, data_folder);
    save([data_folder 'S' subjectID{i} '/jobs_DCM_scrubbing/' 'extract_VOI_SPL_batch.mat'],'matlabbatch');
    spm_jobman('run', matlabbatch)
    clear matlabbatch

    % extract timeseries for SMG VOI
    matlabbatch = extract_VOI_SMG(subjectID{i}, data_folder);
    save([data_folder 'S' subjectID{i} '/jobs_DCM_scrubbing/' 'extract_VOI_SMG_batch.mat'],'matlabbatch');
    spm_jobman('run', matlabbatch)
    clear matlabbatch

    % move the extracted time series to the results folder
    movefile([data_folder 'S' subjectID{i} '/jobs_DCM_scrubbing/' 'VOI*.mat'],[timeseries_folder 'S' subjectID{i}])
...
spm_mat_file = {[data_folder 'S' ID '/jobs_DCM_scrubbing/' 'SPM.mat']};
