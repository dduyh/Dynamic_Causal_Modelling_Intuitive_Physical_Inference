...
b(:,:,simFall) = ones(nregions,nregions); % simFall
...
for i = 1:nsubjects
    ...
    % names = {'instructions','object','simFall','conFall','where','buttonPress','NObP'};
    include = [0 0 1 0 0 0 0]';
...
%     spm_dcm_estimate([glm_dir 'DCM_full.mat']);
...
dcms = spm_select('FPListRec',Data_Folder,'DCM_full.mat');
...
% Check if GCM_full.mat exists
if exist(fullfile(out_dir,'GCM_full_peb.mat'),'file')
    % if GCM_full.mat exists, ask whether to Overwrite
    opts.Default = 'No';
    opts.Interpreter = 'none';
    f = questdlg('Overwrite existing GCM?','Overwrite?','Yes','No',opts);
...
if tf
    ...
    % Save estimated GCM
    save([out_dir '/GCM_full_peb.mat'],'GCM');
end

%% Run diagnostics
load([out_dir '/GCM_full_peb.mat'],'GCM');
spm_dcm_fmri_check(GCM);
